...
sub main {
    ...
    # 获取实例数据目录
    my $dataDir = $self->{dbbackupObj}->getInstanceDataDir($defaultHost,$defaultPort,$dbType,$dbh);
    ...
    if ( $isExistBackupset == 0 ){
        ...
        # 执行备份
        my $result = $self->runBackup($host,$port,$defaultHost,$defaultPort,$defaultDbname,$defaultDbuser,$defaultDbpass,
            $dbType,$bakType,$filerDir,$bakDir,$fileDir,$isSlave,$isEncrypted,$isCompressed,$recordId,
            $flushTableLockLimit,$detailLog,$lvmSpeed,$dbh,$startTime,$dataDir);
    } else {
...
# @Return:  $row or undef
sub runBackup {
    my ( $self,$host,$port,$defaultHost,$defaultPort,$defaultDbname,$defaultDbuser,$defaultDbpass,
        $dbType,$bakType,$filerDir,$bakDir,$fileDir,$isSlave,$isEncrypted,$isCompressed,$recordId,
        $flushTableLockLimit,$detailLog,$lvmSpeed,$dbh,$startTime,$dataDir ) = @_;
    ...
    my $isLocked = 0;
    if ( lc($dbType) =~ /mysql/ ){
        $isLocked = $self->{dbbackupObj}->setMysqlInstanceLock($defaultHost,$defaultPort,$dbType,$dbh,$dataDir,300);
        if ( $isLocked == 0 ){
            $log->error("get mysql lock failed");
        }
    }
    ...
    # 获取实例数据对应的文件系统目录和挂载点
    ( $fileSystem,$mountPoint ) = $self->{dbbackupObj}->getLvmFsMp($defaultHost,$defaultPort,$dbType,$dbh,$dataDir);
    ...
    if ( lc($dbType) =~ /mysql/ ){
        ...
        my $isUnlocked = 0;
        if ( $isSnapshotCreated == 0 ){
                
            # unlock and exit
            $log->error("create snapshot failed, unlock mysql");
            
            # 删除实例锁
            $isUnlocked = $self->{dbbackupObj}->removeInstanceLock($defaultHost,$defaultPort,$dbh);
            if ( $isUnlocked == 0 ){
                $log->error("unlock mysql failed");
                $log->error("please mannul kill backup session");
            }else{
                $log->info("unlock mysql and kill backup session success");
            }
        }
    }
    ...
    # 挂载lvm分区
    my ( $backupMountPoint,$isMounted ) = $self->{dbbackupObj}->mountSnapshot($defaultHost,$defaultPort,$dbType,$filesystemType,$lvPath,$lvName);
    if ( $isMounted == 0 ){
