...
sub main {
    ...
    # 获取实例数据目录
    my $dataDir = $self->{dbbackupObj}->getMysqlInstanceDataDir($dbh);
...
sub runBackup {
    ...
    # 获取实例数据对应的文件系统目录和挂载点
    ( $fileSystem,$mountPoint ) = $self->{dbbackupObj}->getLvmFsMp($defaultHost,$defaultPort,$dbType,$dataDir);
